...
#pragma comment(lib,"Winmm.lib")//mciSendString函数头文件
...
IMAGE img_peashooter_selector_background_left;		//选角界面朝左的豌豆射手背景图片
IMAGE img_peashooter_selector_background_right;		//选角界面朝右的豌豆射手背景图片
IMAGE img_sunflower_selector_background_left;		//选角界面朝左的龙日葵背景图片
IMAGE img_sunflower_selector_background_right;		//选角界面朝右的龙日葵背景图片
...
Atlas atlas_peashooter_idle_left;		//豌豆射手向左默认动画图集
Atlas atlas_peashooter_idle_right;		//豌豆射手向右默认动画图集
Atlas atlas_peashooter_run_left;		//豌豆射手向左奔跑动画图集
Atlas atlas_peashooter_run_right;		//豌豆射手向右奔跑动画图集
Atlas atlas_peashooter_attack_ex_left;	//豌豆射手朝左的特殊攻击动画图集
Atlas atlas_peashooter_attack_ex_right;	//豌豆射手朝右的特殊攻击动画图集
Atlas atlas_peashooter_die_left;		//豌豆射手朝左的死亡动画图集
Atlas atlas_peashooter_die_right;		//豌豆射手朝右的死亡动画图集

Atlas atlas_sunflower_idle_left;		//龙日葵向左默认动画图集
Atlas atlas_sunflower_idle_right;		//龙日葵向右默认动画图集
Atlas atlas_sunflower_run_left;			//龙日葵向左奔跑动画图集
Atlas atlas_sunflower_run_right;		//龙日葵向右奔跑动画图集
Atlas atlas_sunflower_attack_ex_left;	//龙日葵朝左的特殊攻击动画图集
Atlas atlas_sunflower_attack_ex_right;	//龙日葵朝右的特殊攻击动画图集
Atlas atlas_sunflower_die_left;			//龙日葵朝左的死亡动画图集
Atlas atlas_sunflower_die_right;		//龙日葵朝右的死亡动画图集

IMAGE img_pea;							//豌豆图片
Atlas atlas_pea_break;					//豌豆破碎动画图集
Atlas atlas_sun;						//普通阳光动画图集
Atlas atlas_sun_explode;				//普通阳光爆炸动画图集
Atlas atlas_sun_ex;						//特殊阳光动画图集
Atlas atlas_sun_ex_explode;				//特殊阳光爆炸动画图集
Atlas atlas_sun_text;					//“日”文字动画图集
...
IMAGE* img_player_1_avatar = nullptr;//状态条旁的头像图片
IMAGE* img_player_2_avatar = nullptr;

void flip_atlas(Atlas& src, Atlas& dst)//将图集反转
{
	dst.clear();
	for (int i = 0; i < src.get_size(); i++)
	{
		IMAGE img_flpipped;
		flip_image(src.get_image(i), &img_flpipped);//反转
		dst.add_amage(img_flpipped);//添加
	}
}
...
int main()
{
	...
	setbkmode(TRANSPARENT);//文本的背景色为透明

	BeginBatchDraw();//渲染画面流畅

	menu_scene= new MenuScene();//父类指针指向子类对象
	...
	while (true)
	{
		...
		static DWORD last_tick_time = GetTickCount();//stactic 
		DWORD current_tick_time = GetTickCount();
		DWORD delta_tick = current_tick_time - last_tick_time;//这次调用on_update和上次on_update的时间间隔
