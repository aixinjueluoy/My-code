...
inline void flip_image(IMAGE* src, IMAGE* dst)//将面向左的图片翻转
{
	int w = src->getwidth();
	int h = src->getheight();
	Resize(dst, w, h);
	...
	for (int y = 0; y < h; y++)
	{
		for (int x = 0; x < w; x++)
		{
			int idx_src = y * w + x;//翻转前的原图片下标
			int idx_dst = y * w + (w - x - 1);//目标图片下标
...
inline void line(const Camera& camera, int x1, int y1, int x2, int y2)//画出碰撞平台的线
{
	const Vector2& pos_camera = camera.get_position();
	line((int)(x1 - pos_camera.x), (int)(y1 - pos_camera.y), (int)(x2 - pos_camera.x), (int)(y2 - pos_camera.y));
}

inline void sketch_img(IMAGE* src, IMAGE* dst)//原图片   剪影后的白图片
{
	int w = src->getwidth();
	int h = src->getheight();
	Resize(dst, w, h);
	...
	for (int y = 0; y < h; y++)
	{
		...
		{
			int idx = w * y + x;
			dst_buffer[idx] = BGR(RGB(255, 255, 255)) | (src_buffer[idx] & 0xFF000000);//读取目标图片的像色缓冲区并设置为白色
		}
	}
}